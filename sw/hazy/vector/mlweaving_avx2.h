...
void inline retrieve_from_mlweaving(FVector<uint8_t> & dest, uint32_t *src, uint32_t index, uint32_t num_bits, uint32_t num_samples) 
{	
	//dest's information
...
void inline retrieve_from_mlweaving(FVector<uint8_t> & dest, uint32_t *src, uint32_t index, uint32_t num_bits) 
{	
	//dest's information
	uint32_t numFeatures        = dest.size;
	uint8_t* vec_char           = dest.values;
