...
//#include "sample.h"
#include "../mlweaving_8mr_avx2.h" //  mlweaving_8mr


//The related parameters are here. 

#define NUM_FEATURES          780   //5017  //512 *3//
#define NUM_SAMPLES           51492 //6545 //512 *3//
...
int test_mlweaving(uint32_t *data, uint32_t *data_bitweaving, uint32_t num_bits )
{
  ...
  for (uint32_t i = 0; i < NUM_SAMPLES; i++)   
  {
    ...
    hazy::vector::retrieve_from_mlweaving(dest_v, data_bitweaving, i, num_bits, NUM_SAMPLES);
    for (uint32_t j = 0; j < NUM_FEATURES; j++)
    {
      if ( ((data[i*NUM_FEATURES+j]>>shift_bits)&T_mask) != dest_v[j] )
      {
        printf("Error (%d-bit): sample_%d,feature_%d: src_0x%8x, T_mask_0x%8x, dest_0x%8x\n", num_bits, i, j, (data[i*NUM_FEATURES+j]>>shift_bits), T_mask, dest_v[j]);
        return -1;
      }    
    }    
  }
